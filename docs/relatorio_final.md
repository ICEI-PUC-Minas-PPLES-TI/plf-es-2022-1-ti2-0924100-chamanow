...
| Avaliação do Cliente | Fornecer a avaliação média de um cliente à um prestador | Calcula a média de avaliações de cada cliente | (∑(avaliacao_usuario(nota) e avaliacao_usuario(cod_avaliado))  | avaliacao_usuario | Cliente |
...
| Cancelamento | Identificar processos passíveis de melhorias mensalmente | Percentual de processos cancelados em relação ao total de serviços executados |  (∑(agendamento(status) concluído) / ∑(agendamento(status) = cancelado) * 100  | agendamento | Otimização de processos |
