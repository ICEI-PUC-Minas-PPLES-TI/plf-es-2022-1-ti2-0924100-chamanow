...
const servPendente = '';
connection.query("SELECT COUNT(CASE WHEN STATUS LIKE 'Pendente' THEN 1 END) / COUNT(CASE WHEN COD_TIPO IS NOT NULL THEN 1 END) *100 AS SERVICOS_PENDENTES FROM AGENDAMENTO GROUP BY MONTH(DATA_SOLICITACAO)", { type: QueryTypes.SELECT}).then(servPendente=>{console.log("SERVIÇOS PENDENTES",servPendente)});
