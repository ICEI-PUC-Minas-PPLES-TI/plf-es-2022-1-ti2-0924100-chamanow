...
| Avaliação do Cliente | Fornecer a avaliação média de um cliente à um prestador | Calcula a média de avaliações de cada cliente | (∑(avaliacao_usuario(nota) e avaliacao_usuario(cod_avaliado)) / Count(avalicao_usuario(nota))  | avaliacao_usuario | Cliente |
...
| Serviços agendados |  Avaliar o crescimento da plataforma | Total de serviços agendados mês à mês |  (∑(agendamento(data_solicitacao))/∑(agendamento(cod_servico))) * 100  | agendamento | Processos internos |
